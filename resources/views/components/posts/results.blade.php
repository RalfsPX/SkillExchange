...
@if ($posts->isEmpty())
    <div
        class="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
        {{ $filtered ? 'No posts match your search.' : 'No posts yet. Be the first to create one.' }}</div>
@else
        ...
        @foreach ($posts as $post)
            ...
            <a href="{{ route('posts.show', $post) }}" wire:key="post-{{ $post->id }}"
                ...
                <div
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ $post->user->username }} &middot;
